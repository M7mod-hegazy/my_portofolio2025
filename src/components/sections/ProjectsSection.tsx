...
import { ExternalLink, Github, X, ChevronLeft, ChevronRight, Zap, Users, Calendar, Smartphone, Monitor, Cpu, Code, Terminal, Film, ChevronDown, Activity, Link2, Check } from "lucide-react";
...
export const ProjectsSection = () => {
  ...
  const handleCopyCurrentProjectLink = async () => {
    if (!activeProject) return;
    const shareUrl = `${window.location.origin}${window.location.pathname}?project=${activeProject._id}`;
    try {
      await navigator.clipboard.writeText(shareUrl);
      setCopiedProjectLink(true);
      toast.success("Project link copied");
      window.setTimeout(() => setCopiedProjectLink(false), 1500);
    } catch {
  ...
  return (
    <section id="projects" className="relative min-h-screen py-20 bg-background overflow-hidden selection:bg-orange-500/30">
        ...
        {isLoading ? (
          <div className="min-h-[400px] flex items-center justify-center">
            <div className="text-orange-500 font-mono animate-pulse flex flex-col items-center">
              ...
              <span>SYNCING_PROJECT_DATABASE...</span>
            </div>
          </div>
        ) : (
          <>
      ...
      <Dialog open={!!activeProjectId} onOpenChange={(open) => !open && closeProject()}>
          ...
          {!activeProject ? (
            ...
            <>
              ...
              <div className="w-full md:w-1/3 md:h-full bg-background flex flex-col shrink-0">
                <div className="px-6 pt-6 pb-3 flex justify-end items-center gap-2 pr-12">
                  <Button
                    type="button"
                    onClick={handleCopyCurrentProjectLink}
                    className={`h-9 px-3 font-mono text-xs rounded-sm ${copiedProjectLink ? "bg-emerald-600 hover:bg-emerald-600 text-white" : "bg-cyan-600 hover:bg-cyan-500 text-white"}`}
                  >
                    {copiedProjectLink ? <Check className="w-3.5 h-3.5 mr-1.5" /> : <Link2 className="w-3.5 h-3.5 mr-1.5" />}
